...
export default () => {
    var personApi = router();
    personApi.get('/api/person/:page_no/:page_size/:filter/:id_company', (req, resp) => {
        ...
        CompanyModel.findOne(filterCondition).then(
            company => {
                CompanyModel.aggregate(pipe).exec()
                    .then(companyResult => {
                        const page = {
    ...
    personApi.post('/api/person/', (req, resp) => {
        if (!req.body) {
            return res.status(400).send('Request body is missing')
        }
        CompanyModel.findOneAndUpdate({ _id: req.body.id_company }, { $push: { persons: req.body.person } })
            .then(company => {
    ...
    personApi.put('/api/person/', (req, resp) => {
        if (!req.body) {
            return res.status(400).send('Request body is missing')
        }
    ...
    personApi.delete('/api/person/:id/:id_company', (req, resp) => {
